...
final certificateNotifierProvider = StateNotifierProvider<CertificateNotifier, AsyncValue<List<Certificate>>>((ref) {
  ...
  return CertificateNotifier(ref, repository);
});
...
class CertificateNotifier extends StateNotifier<AsyncValue<List<Certificate>>> {
  ...
  CertificateNotifier(this._ref, this._repository) : super(const AsyncValue.data([])) {
    fetchCertificates();
  }
  ...
  // ✅ Refrescar solo estadísticas de admin
  ...
  Future<void> createCertificate(Map<String, dynamic> data) async {
    ...
    try {
      ...
      await _refreshAdminStats(); // ✅ Refrescar estadísticas admin
    } catch (e, stack) {
      state = AsyncValue.error(e, stack);
    }
  }
...
final certificateByIdProvider = FutureProvider.family<Certificate, int>((ref, id) {
  final repository = ref.watch(teacherRepositoryProvider).certificates;
